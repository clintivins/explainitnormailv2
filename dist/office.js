(function () {
  ...
  const TYPE_META = {
    ...
    "ceo.note": { state: "noting", chip: "Reviewing next title", icon: "🗂" },
  ...
  function boot() {
    ...
    const tickerScene = document.getElementById("office-ticker");
    const tickerHome = document.getElementById("office-ticker-cards");
    const tickerEl = tickerScene || tickerHome;
    const homeTicker = tickerHome && tickerHome !== tickerEl ? tickerHome : null;
    ...
    const tickerBar = document.querySelector(".office-ticker-bar") || document.querySelector(".office-livebar");
    ...
    const hasOfficeScene = stations.length > 0 || Boolean(feedEl || tickerEl || modeLabel || sourceLine);
    ...
    function setMode(next, detail) {
      ...
      if (modeLabel) {
        if (next === "live") {
          modeLabel.textContent = "Live from the office";
    ...
    function pulseBar() {
      ...
      tickerBar.classList.remove("livebar-pulse");
      void tickerBar.offsetWidth;
      tickerBar.classList.add("livebar-pulse");
    }
    ...
    function chipFor(station, actor) {
      return (
        (station && station.querySelector(".status-chip")) ||
        document.querySelector("[data-chip-for='" + actor + "']")
      );
    }
    ...
    function applyEventToStation(event) {
      if (event.type === "idle" && event.actor === "system") {
        stations.forEach(function (station) {
          ...
          const chip = chipFor(station, actor);
          if (chip) {
      ...
      const fallbackMs = (meta.state === "shipped" || event.type === "chat")
        ? (event.type === "chat" ? QUOTE_MS : SHIPPED_FALLBACK_MS)
        : hideMs;
      ...
      if (fallbackMs) {
        stationTimers[actor] = window.setTimeout(function () {
          ...
          if (meta.state === "shipped" || event.type === "chat") {
            ...
            const currentChip = chipFor(current, actor);
            if (currentChip) {
              currentChip.textContent = "Idle";
            }
          }
        }, fallbackMs);
      }
    ...
    function applyLatestStationStates() {
    ...
    function renderHomeChips() {
      ...
      ["author", "ceo", "social"].forEach(function (actor) {
        ...
        if (!el || el.closest("[data-actor-station]")) {
          return;
        }
    ...
    function renderTicker() {
      ...
      if (!events.length) {
        const empty = document.createElement("div");
        empty.className = "live-empty";
        empty.textContent = "Waiting for the first office note.";
    ...
    async function loadLocalEvents() {
      const sources = ["office-events.json", "./office-events.json"];
    ...
    function startPolling() {
      ...
      const tick = function () {
        pullRemoteEvents().catch(function () {
          if (mode !== "live") {
            startFallback("Could not reach the events API. Showing the office preview.");
          }
        });
      };
    ...
    async function startFallback(detail) {
      if (mode === "live") {
        return;
      }
    ...
    function armApiGiveUp() {
      ...
      apiGiveUpTimer = window.setTimeout(function () {
        startFallback("Events API did not respond in time. Showing the office preview.");
      }, SSE_WAIT_MS);
    }
    ...
    async function connect() {
      const base = getEventsBase();
      if (!base) {
        await startFallback("Offline preview — set NEXT_PUBLIC_OFFICE_EVENTS_URL to stream live events.");
        return;
      }

      setMode("connecting", "Connecting to the live office feed…");
      armApiGiveUp();
      try {
        ...
        apiGiveUpTimer = null;
        if (count === 0) {
          await startFallback("No live events yet. Showing the office preview.");
        }
      } catch (_error) {
        window.clearTimeout(apiGiveUpTimer);
        apiGiveUpTimer = null;
        await startFallback("Could not reach the events API. Showing the office preview.");
      }
      startSse();
    }
