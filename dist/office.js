(function () {
  ...
  function getApiBase() {
    const raw = window.NEXT_PUBLIC_OFFICE_EVENTS_URL;
  ...
  function getFetchSources() {
    ...
    const remote = base ? [base + "/api/office/events?limit=40"] : [];
    // Keep local fallbacks for static hosting and offline demos.
    return remote.concat([
      "office-events.json",
      "/office-events.json",
      "/api/office/events?limit=40"
    ]);
  }
  ...
  function getSseUrl() {
    const base = getApiBase();
    if (base) {
      return base + "/api/office/events/stream";
    }
    return "/api/office/events/stream";
  }
